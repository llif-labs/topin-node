...
const IssueCertService = {
  ...
  getMyIssues: async (req, res) => {
    ...
    try {
      ...
      statusResponse(req, res, STATUS.SUCCESS.code, STATUS.SUCCESS.message, issues);
    } catch (e) {
      statusResponse(req, res, STATUS.BAD_REQUEST.code, e.message);
    }
  },
  ...
  deleteIssue: async (req, res) => {
    ...
    try {
      ...
      statusResponse(req, res, STATUS.NO_CONTENT.code, STATUS.NO_CONTENT.message);
    } catch (e) {
      statusResponse(req, res, STATUS.BAD_REQUEST.code, e.message);
    }
  },
  ...
  participate: async (req, res) => {
    ...
    try {
      ...
      // 이미 참여했는지 확인
      const participation = await dbConn.query(IssueRepository.checkParticipation, [user_id, issueId]);
      if (participation.length > 0) {
        return statusResponse(req, res, STATUS.CONFLICT.code, '이미 이 이슈에 참여했습니다.');
      }

      // 이슈가 승인된 상태인지 확인
      if (!issue[0].is_approved) {
